...
function createPage(){
    const  {viewName, endpointName, section, unique_url } = router.getCurrentState();
    ...
    if(unique_url=='order')
    {
        ...
        import (('./order.js'))
            ...
            .then((data)=>{
                templateProcessor.render(view(data, GetBooksInCart() ));
            });
    }
...
function SetBooksInCart(data)
{
    books_in_cart= books_in_cart||[];
    books_in_cart.push(data);
}

function SetBooksInCartJSON(data)
{
    books_in_cart= books_in_cart||[];
    books_in_cart=JSON.parse(data);
}
...
function SetDataStorageItem(elm, arr)
{
    data_storage.setItem(elm,JSON.stringify(arr));
}
...
async function getPrice(){
    ...
    response['books'].map((catalogname) =>{
        catalogname[Object.keys(catalogname)].map((blockelement) =>{
            SetUniqueID(blockelement.uniqueID);
            SetPrice(blockelement.price);
        });
    }
    );
}
...
window.addEventListener('load', () => {
    ...
    SetDataStorage();
    let booksInCart;
    let arr_cart=GetDataStorage().getItem('books_in_cart');
    SetBooksInCartJSON(arr_cart);
    booksInCart=JSON.parse(arr_cart);
    if(booksInCart==[])
    {
        renewQuantity(0);
    }
    ...
    {
        renewQuantity(booksInCart.length);
    }
...
function save_cart()
{
    SetDataStorageItem('books_in_cart', books_in_cart);
...
function show_order()
{
    let booksInCart= GetBooksInCart();
    for ( let i=0;i<booksInCart.length;i++)booksInCart.splice(i);
    renewQuantity(booksInCart.length);
    clear_cart();
...
window.onclick = function (event) {
    ...
    if(event.target.id.substr(0,3)=='del')
    {
        ...
        {
            if(event.target.id==`del${booksInCart[i]}`)
            {
    ...
    for(let i= 0; i<GetUniqueID().length; i++) {
    ...
    for(let i= 0; i<uniqueID.length; i++) {
        ...
        if (uniqueID[i].split('').reverse().join('') === event.target.id) {
            SetBooksInCart(uniqueID[i]);
            renewQuantity(GetBooksInCart().length);
...
function check_all_fields(){
    ...
    if(ValidName()&&ValidMail()&&ValidPhone()&&GetBooksInCart().length>0)
    {
